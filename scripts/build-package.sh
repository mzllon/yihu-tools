...
install -m755 "$target/release/yihu" "$target/release/autodark-agent" "$stage/$name/"
...
(cd "$stage/$name"; sha256sum yihu autodark-agent icons/*.png ARCH VERSION BUILD_EXE reinstall.sh > SHA256SUMS
